...
%
%                   初版作者：  许笑颜 (XIDIAN UNIVERSITY，2024.03.21)
%                   指导者:     罗翌新 (USTC)
%                   NOTE:       i=1 对应 x=0, i=nx 对应 x=L
...
int main()
{
    ...
    for(Va_cnt = 0; Va_cnt <=num_V +1;Va_cnt++){  //+1 b/c 1st Va is the equil run
        not_converged = false;  // 记录是否收敛
        not_cnv_cnt = 0;        // 记录未收敛的次数。
        if(tolerance > 1e-5){
        ...
        while(error_np > tolerance){
            ...
            set_bp(p_mob, B_p1, B_p2, p_leftBC, p_rightBC, Up, rhs); //更新了Up与rhs
            oldp = p;
